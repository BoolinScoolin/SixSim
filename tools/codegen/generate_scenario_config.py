...
def render_fcu_config_paths(config):
    profile_names = []
    for profile in config["fcu_profiles"]:
        if profile["name"] not in profile_names:
            profile_names.append(profile["name"])
    return "\n".join(
        "  scenario.fcu_config_paths.emplace_back("
        + cpp_string(
            str((REPO_ROOT / "configs" / "fcus" / f"{profile_name}.yaml").resolve())
        )
        + ");"
...
{render_fcu_config_paths(config)}
